...
use reticulum_appliance_store::{
    AcceptanceIds, AcceptanceOutcome, ChatStore, Contact, ContactUpsertOutcome, ConversationPeer,
    DestinationHash, IdempotencyKey, InboundCommitOutcome, MessageActivityPage,
    MessageActivityPageRequest, MessageId, OutboxCommitOutcome, OutboxId, OutboxMaterial,
    OutboxRetryOutcome, PhoneLocationSample, ReconcileWork, StatusProjectionOutcome,
    SubmissionState, TimelineEntry,
};
...
impl<S: ChatStore> ChatEngine<S> {
    ...
    /// Create a replacement device submission for a retryable terminal row
    /// without creating a second timeline entry or changing its signed LXMF
    /// material and message identity.
    pub fn retry_send(
        &mut self,
        outbox_id: OutboxId,
