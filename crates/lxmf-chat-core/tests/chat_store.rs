use reticulum_lxmf_chat_core::{
    ...
    MAX_MESSAGE_ACTIVITY_EVENTS, MemoryChatStore, MessageActivityKind, MessageActivityPageRequest,
    MessageActivityRetryTrigger, MessageActivityScope, MessageId, MessageIngressObservation,
    MessageInterfaceId, MessageLocation, MessageSignalObservation, OutboxCommitOutcome,
    OutboxMaterial, OutboxRetryOutcome, OutboxStatus, PacketEvidence, PhoneLocationAuthorization,
    PhoneLocationSample, PhoneLocationSource, PhoneLocationUnavailableReason, ReconcileWork,
    RfTraceAttemptObservation, RfTraceAttemptOutcome, RfTraceBootId, RfTraceEventSequence,
    ...
    StatusProjectionOutcome, SubmissionFailure, SubmissionId, SubmissionState, TimelineDirection,
    UnixTimestampMillis,
};
...
#[test]
fn automatic_retry_budget_persists_and_manual_retry_remains_available_after_exhaustion() {
    let mut store = MemoryChatStore::new();
    let original = outbound(0x61, 2, 13_000, b"bounded eventual");
    let id = store.commit_outbound(original).unwrap().outbox_id();
    assert_eq!(
        store.outbox(id).unwrap().unwrap().automatic_retry_count(),
        0
    );

    for automatic_retry in 0..AUTOMATIC_OUTBOX_RETRY_LIMIT {
        let accepted = acceptance(u64::from(70 + automatic_retry), 0xa2);
        store.record_acceptance(id, accepted).unwrap();
        store
            .project_submission_status(
                accepted.submission_id(),
            ...
            .unwrap();
        assert_eq!(store.retryable_outbox().unwrap().len(), 1);
        assert_eq!(
            store
                .retry_outbox_automatically(id, IdempotencyKey::new([0x62 + automatic_retry; 16]),)
                .unwrap(),
        ...
        store = MemoryChatStore::open(store.image()).unwrap();
        assert_eq!(
            store.outbox(id).unwrap().unwrap().automatic_retry_count(),
            automatic_retry + 1
        );
    ...
    let final_acceptance = acceptance(80, 0xa2);
    ...
    let before = store.outbox(id).unwrap().unwrap();
    assert_eq!(
        store
            .retry_outbox_automatically(id, IdempotencyKey::new([0x66; 16]))
            .unwrap(),
        AutomaticOutboxRetryOutcome::BudgetExhausted(id)
    );
    assert_eq!(store.outbox(id).unwrap().unwrap(), before);

    assert_eq!(
        store
            .retry_outbox(id, IdempotencyKey::new([0x70; 16]))
            .unwrap(),
        OutboxRetryOutcome::Requeued(id)
    );
    let manually_requeued = store.outbox(id).unwrap().unwrap();
    assert_eq!(manually_requeued.status(), OutboxStatus::Committed);
    assert_eq!(
        manually_requeued.automatic_retry_count(),
...
fn outbound_attempt_locations_are_atomic_exact_once_and_restart_safe() {
    ...
    store
        ...
        .unwrap();
    let automatic_location = location(46_234_567, 19_200);
    assert_eq!(
        store
            .retry_outbox_automatically_with_location(
                outbox_id,
...
fn message_activity_is_exact_once_attempt_aware_scoped_and_restart_safe() {
    ...
    assert_eq!(outbound.automatic_retry_count(), Some(0));
    assert_eq!(
