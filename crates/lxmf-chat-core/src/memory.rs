...
use crate::{
    ...
    ConversationPeer, DestinationHash, IdempotencyKey, ImageError, InboundCommitOutcome,
    InboundMessage, InboundRecord, MAX_MESSAGE_ACTIVITY_EVENTS, MessageActivityEvent,
    MessageActivityId, MessageActivityKind, MessageActivityPage, MessageActivityPageRequest,
    MessageActivityRetryTrigger, MessageActivityScope, MessageAttemptNumber, MessageId,
...
pub const MEMORY_IMAGE_SCHEMA_VERSION: u16 = 8;
...
impl MemoryChatStore {
    ...
    fn rearm_outbox(
        ...
        if charge_legacy_automatic_budget {
            record.automatic_retry_count = record
                .automatic_retry_count
                .checked_add(1)
                .expect("automatic retry limit must fit its persisted counter");
        }
