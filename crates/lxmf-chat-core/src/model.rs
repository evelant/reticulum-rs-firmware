...
/// Historical app-owned automatic-rearm budget retained for schema migration
/// and store-conformance fixtures.
///
/// Production clients must not schedule from this value. Current firmware owns
/// unattended LXMF retry inside one durable `Preparing` submission.
pub const AUTOMATIC_OUTBOX_RETRY_LIMIT: u8 = 3;
...
///
/// An ambiguous call is replayed with the same key. A transitional explicit
/// retry of a legacy or permanently terminal row uses a fresh key to create a
/// replacement durable device submission while preserving the semantic LXMF
/// material and message identity. Board-owned carrier retries do not rotate it.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
///
/// Fresh databases retain the exact lifetime number. A migrated database may
/// only reconstruct the lower bound implied by its persisted historical
/// automatic-rearm count; [`MessageActivityPage::history_incomplete`] exposes
/// that caveat.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
pub enum MessageActivityKind {
    ...
    OutboundRequeued {
        /// Historical policy or user action that caused the replacement.
        trigger: MessageActivityRetryTrigger,
...
impl MessageActivityPage {
    ...
    /// Whether migration or retention pruning omitted older activity.
...
pub enum SubmissionFailure {
    ...
    NoPath,
    /// Historical or permanent device-submission delivery timeout.
    ///
...
impl SubmissionFailure {
    ...
    /// transitional explicit action for legacy or permanently terminal rows,
    /// not a carrier retry in the board-owned loop. A downstream rejection is
    /// an explicit refusal rather than a lossy-network condition.
...
pub enum SubmissionState {
    ...
    Preparing,
    /// A raw-RNS or legacy attempt was released and is awaiting proof.
...
pub struct OutboxRecord {
    ...
    pub(crate) automatic_retry_count: u8,
...
impl OutboxRecord {
    ...
    /// Historical app-owned automatic rearms already consumed by this row.
    ///
    /// This read-side compatibility count persists across application restarts
    /// but is not a firmware RNS-attempt count. Current production rows do not
    /// increment it.
    pub const fn automatic_retry_count(&self) -> u8 {
...
impl TimelineEntry {
    ...
    pub(crate) fn outbound(record: &OutboxRecord) -> Self {
        Self {
            ...
            automatic_retry_count: Some(record.automatic_retry_count()),
    ...
    ///
    /// This is read-side migration compatibility, not a count of firmware RNS
    /// attempts. Current production rows do not increment it.
