//! Explicit, validated LoRa configuration.
//!
...
/// Inclusive frequency range supported by one fitted radio/antenna path.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoRaFrequencyRange {
    minimum_hz: u32,
    maximum_hz: u32,
}

impl LoRaFrequencyRange {
    /// Validate and construct an inclusive radio-frequency range.
    pub const fn try_new(
        minimum_hz: u32,
        maximum_hz: u32,
    ) -> Result<Self, LoRaFrequencyRangeError> {
        if minimum_hz == 0 {
            return Err(LoRaFrequencyRangeError::ZeroMinimum);
        }
        if minimum_hz > maximum_hz {
            return Err(LoRaFrequencyRangeError::Reversed {
                minimum_hz,
                maximum_hz,
            });
        }
...
/// Invalid board/radio frequency-range definition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LoRaFrequencyRangeError {
...
/// Untrusted numeric configuration supplied by a caller or persistent store.
///
...
/// LoRa settings validated for one board/radio path.
///
...
impl LoRaProfile {
    /// Validate explicit numeric configuration against a fitted RF path.
    pub const fn validate(
        config: LoRaProfileConfig,
        supported_range: LoRaFrequencyRange,
    ) -> Result<Self, LoRaProfileError> {
        let frequency_hz = match config.frequency_hz {
            Some(frequency_hz) => frequency_hz,
            None => return Err(LoRaProfileError::MissingFrequency),
        };
        let spreading_factor = match config.spreading_factor {
            ...
            value => return Err(LoRaProfileError::UnsupportedSpreadingFactor(value)),
        };
        let bandwidth = match config.bandwidth_hz {
            ...
            value => return Err(LoRaProfileError::UnsupportedBandwidthHz(value)),
        };
        let coding_rate = match config.coding_rate_denominator {
            ...
            7 => CodingRate::_4_7,
            8 => CodingRate::_4_8,
            value => return Err(LoRaProfileError::UnsupportedCodingRateDenominator(value)),
        };
        ...
        {
            return Err(LoRaProfileError::ChannelOutsideSupportedRange {
                frequency_hz,
        ...
        if config.preamble_symbols < RNODE_MIN_PREAMBLE_SYMBOLS {
            return Err(LoRaProfileError::PreambleTooShort {
                actual: config.preamble_symbols,
                minimum: RNODE_MIN_PREAMBLE_SYMBOLS,
            });
        }
        if !config.explicit_header {
            return Err(LoRaProfileError::ExplicitHeaderRequired);
        }
        if !config.crc {
            return Err(LoRaProfileError::CrcRequired);
        }
        if config.iq_inverted {
            return Err(LoRaProfileError::NormalIqRequired);
        }
        if frequency_hz < 400_000_000
            && matches!(bandwidth, Bandwidth::_250KHz | Bandwidth::_500KHz)
        {
            return Err(LoRaProfileError::WideBandwidthBelow400MHz {
                frequency_hz,
                bandwidth_hz: config.bandwidth_hz,
            });
        }
        if has_unverified_rnode_ldro(config.spreading_factor, bandwidth) {
            return Err(LoRaProfileError::UnverifiedRnodeLdroCombination {
    ...
    /// airtime. The timeout allows twice that duration plus a five-second
    /// scheduling guard. This scales correctly for narrow-band profiles instead
    /// of using a short fixed timeout that rejects a valid frame before RX
    /// completion.
...
/// Rejection from project-owned LoRa-profile validation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// claiming an interoperability profile until the driver is fixed or a
/// specific combination has been verified on hardware.
...
mod tests {
    ...
    const TEST_RANGE: LoRaFrequencyRange =
        match LoRaFrequencyRange::try_new(863_000_000, 928_000_000) {
            Ok(range) => range,
            Err(_) => panic!("invalid test range"),
        };

    const fn valid_config() -> LoRaProfileConfig {
        LoRaProfileConfig {
    ...
    fn explicit_rnode_profile_is_preserved() {
        let profile = LoRaProfile::validate(valid_config(), TEST_RANGE).unwrap();
    ...
    fn symbol_time_uses_exact_sx126x_bandwidth_divisors_and_rounds_up() {
        ...
        let rounded = LoRaProfile::validate(rounded_bandwidth, TEST_RANGE).unwrap();
        ...
        let slow = LoRaProfile::validate(slow, TEST_RANGE).unwrap();
    ...
    fn fragment_timeout_scales_from_full_frame_airtime() {
        ...
        let slow = LoRaProfile::validate(slow_config, TEST_RANGE).unwrap();
    ...
    fn missing_frequency_never_falls_back() {
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::MissingFrequency)
        );
    }
    ...
    fn fitted_frequency_bounds_are_inclusive_and_enforced() {
        for frequency_hz in [863_062_500, 927_937_500] {
            ...
            assert!(LoRaProfile::validate(config, TEST_RANGE).is_ok());
        }
        ...
        for frequency_hz in [863_062_499, 927_937_501] {
            ...
            assert_eq!(
                LoRaProfile::validate(config, TEST_RANGE),
                Err(LoRaProfileError::ChannelOutsideSupportedRange {
                    frequency_hz,
                    bandwidth_hz: 125_000,
    ...
    fn unsupported_modulation_values_are_rejected() {
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::UnsupportedSpreadingFactor(6))
        );
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::UnsupportedBandwidthHz(100_000))
        );
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::UnsupportedCodingRateDenominator(9))
        );
    }
    ...
    fn rnode_preamble_floor_is_enforced() {
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::PreambleTooShort {
                actual: 17,
                minimum: 18,
            })
        );
    ...
    fn rnode_header_crc_and_iq_are_fixed_for_this_slice() {
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::ExplicitHeaderRequired)
        );
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::CrcRequired)
        );
        ...
        assert_eq!(
            LoRaProfile::validate(config, TEST_RANGE),
            Err(LoRaProfileError::NormalIqRequired)
        );
    }

    #[test]
    fn sx126x_wide_bandwidth_low_frequency_combination_is_rejected() {
        let broad_range = LoRaFrequencyRange::try_new(137_000_000, 1_020_000_000).unwrap();
        for bandwidth_hz in [250_000, 500_000] {
            let mut config = valid_config();
            config.frequency_hz = Some(399_999_999);
            config.bandwidth_hz = bandwidth_hz;
            assert_eq!(
                LoRaProfile::validate(config, broad_range),
                Err(LoRaProfileError::WideBandwidthBelow400MHz {
    ...
    fn unverified_rnode_ldro_combinations_are_rejected() {
        ...
        for (spreading_factor, input_bandwidth_hz, canonical_bandwidth_hz) in cases {
            ...
            assert_eq!(
                LoRaProfile::validate(config, TEST_RANGE),
                Err(LoRaProfileError::UnverifiedRnodeLdroCombination {
                    spreading_factor,
                    bandwidth_hz: canonical_bandwidth_hz,
                })
            );
        ...
        assert!(LoRaProfile::validate(verified, TEST_RANGE).is_ok());
    }

    #[test]
    fn invalid_frequency_range_definitions_are_rejected() {
        assert_eq!(
            LoRaFrequencyRange::try_new(0, 928_000_000),
            Err(LoRaFrequencyRangeError::ZeroMinimum)
        );
        assert_eq!(
            LoRaFrequencyRange::try_new(928_000_000, 863_000_000),
            Err(LoRaFrequencyRangeError::Reversed {
