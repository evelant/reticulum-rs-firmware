...
/// Normalize an EUI-48 expressed with optional `:` or `-` separators.
pub fn normalize_eui48(value: &str) -> Option<String> {
...
/// Parse a normalized or separator-delimited EUI-48.
pub fn parse_eui48(value: &str) -> Option<[u8; 6]> {
    let normalized = normalize_eui48(value)?;
    let mut eui48 = [0_u8; 6];
    hex::decode_to_slice(normalized, &mut eui48).ok()?;
    Some(eui48)
}

/// Owner-private root containing one profile directory per board EUI-48.
...
impl ProfileRoot {
    ...
    /// Return the normalized EUI-48 values with existing profile directories.
    pub fn existing_eui48s(&self) -> Result<Vec<String>, String> {
        let devices = self.path.join(DEVICES_DIRECTORY);
        reject_symlink_ancestors(&devices)?;
        require_private_directory(&devices, "profile devices directory")?;
        let mut eui48s = Vec::new();
            ...
            .map_err(|error| format!("could not read {}: {error}", devices.display()))?
        {
            ...
            let normalized = normalize_eui48(&name).ok_or_else(|| {
                format!(
            ...
            eui48s.push(normalized);
        }
        eui48s.sort_unstable();
        Ok(eui48s)
    }

    /// Create or reopen the profile for one exact board EUI-48.
    pub fn device(&self, eui48: &str) -> Result<DeviceProfile, String> {
        let eui48 = normalize_eui48(eui48).ok_or_else(|| {
            "board EUI-48 must contain exactly twelve hexadecimal digits".to_owned()
        })?;
        let directory = self.path.join(DEVICES_DIRECTORY).join(&eui48);
        ...
        Ok(DeviceProfile { eui48, directory })
    }
}
...
impl DeviceProfile {
    /// Canonical twelve-digit board EUI-48.
    pub fn eui48(&self) -> &str {
        &self.eui48
    }
...
mod tests {
    ...
    fn temporary_root() -> PathBuf {
        ...
        std::env::temp_dir().canonicalize().unwrap().join(format!(
            "reticulum-appliance-profile-test-{}-{serial}",
            std::process::id()
        ))
    }
    ...
    fn profiles_use_canonical_non_secret_paths() {
        ...
        assert_eq!(profile.eui48(), "ACA704E13E88");
        assert_eq!(
            profile.credential_path(),
            path.join("devices/ACA704E13E88/credential.rdpkey")
        );
        assert_eq!(root.existing_eui48s().unwrap(), vec!["ACA704E13E88"]);
    ...
    fn invalid_or_noncanonical_profile_entries_fail_closed() {
        let path = temporary_root();
        let root = ProfileRoot::open(&path).unwrap();
        fs::create_dir(path.join("devices/not-an-eui48")).unwrap();

        assert!(root.existing_eui48s().is_err());
        fs::remove_dir_all(path).unwrap();
    }
