...
fn run(arguments: impl Iterator<Item = String>) -> Result<String, String> {
    ...
    let check = match arguments.as_slice() {
        ...
        _ => {
            return Err(
                "usage: cargo run -p reticulum-appliance-service --bin export-api-bindings -- [--check]"
                    .to_owned(),
            );
        }
    };
    ...
    let generated = reticulum_appliance_service::render_api_bindings();
    if check {
        ...
        if committed != generated {
            return Err(
                "generated bindings are stale; run `cargo run --locked -p reticulum-appliance-service --bin export-api-bindings`"
                    .to_owned(),
            );
        }
        return Ok(format!(
