//! Bounded synchronous adapter for the appliance BLE GATT profile.
...
/// BLE discovery, I/O, and selection policy for one appliance connection.
#[derive(Clone, Debug)]
...
impl BleTransport {
    ...
    pub fn connect(config: BleConfig) -> Result<(Self, SelectedPeripheral), String> {
        ...
        let worker = thread::Builder::new()
            .name("reticulum-host-ble".to_owned())
            .spawn(move || {
...
/// Portable diagnostic stub on hosts where the service intentionally does not
/// link a platform BLE backend.
...
impl BleTransport {
    ...
    pub fn connect(_config: BleConfig) -> Result<(Self, SelectedPeripheral), String> {
        Err("the direct BLE host adapter is available only on macOS".to_owned())
    }
