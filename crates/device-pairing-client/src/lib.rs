//! Transport-neutral client state for the resident Reticulum device-pairing
//! lifecycle.
//!
//! Transport integrations construct a [`PairingSession`] over any blocking
//! [`PairingStream`]. The session keeps one stream, decoder, bearer binding,
//! and exact-next sequence owner alive across status, optional initialization,
//! Begin, ProofStart, and Activate.
//!
        ...
        match response {
            ...
            PairingResponse::ProofStart(ProofStartResponse::Failure { failure, .. }) => {
                let reconciliation = if resumed {
                    " the retained Pending artifact may no longer match device state; do not guess or abort;"
                } else {
                    ""
                };
                return Err(format!(
