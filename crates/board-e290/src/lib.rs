...
use reticulum_radio_interface::{
    LoRaFrequencyRange, LoRaProfile, LoRaProfileConfig, LoRaProfileError,
};
...
/// Inclusive frequency range for the fitted HT-RA62-HF radio path.
pub const LORA_FREQUENCY_RANGE: LoRaFrequencyRange =
    match LoRaFrequencyRange::try_new(RF_MATCHING_MIN_HZ, RF_MATCHING_MAX_HZ) {
        Ok(range) => range,
        Err(_) => panic!("invalid Vision Master E290 RF range"),
    };

/// Validate an explicit LoRa profile against the complete fitted radio path.
///
...
pub const fn validate_lora_profile(
    config: LoRaProfileConfig,
) -> Result<LoRaProfile, LoRaProfileError> {
    LoRaProfile::validate(config, LORA_FREQUENCY_RANGE)
}
...
mod tests {
    ...
    fn valid_profile(frequency_hz: u32) -> LoRaProfileConfig {
        LoRaProfileConfig {
    ...
    fn fitted_range_and_design_memory_floor_are_exact() {
        ...
        assert_eq!(LORA_FREQUENCY_RANGE.minimum_hz(), 863_000_000);
        assert_eq!(LORA_FREQUENCY_RANGE.maximum_hz(), 928_000_000);
    ...
    fn profile_validation_requires_the_complete_channel_inside_fitted_range() {
        ...
        assert!(validate_lora_profile(valid_profile(lower_valid_center)).is_ok());
        assert!(validate_lora_profile(valid_profile(915_000_000)).is_ok());
        assert!(validate_lora_profile(valid_profile(upper_valid_center)).is_ok());

        let below = lower_valid_center - 1;
        assert_eq!(
            validate_lora_profile(valid_profile(below)),
        ...
        assert_eq!(
            validate_lora_profile(valid_profile(above)),
            Err(LoRaProfileError::ChannelOutsideSupportedRange {
                frequency_hz: above,
                bandwidth_hz: 125_000,
