...
use reticulum_board_e290::validate_lora_profile;
use reticulum_radio_interface::{
    BoundedRxOutcome, CadObservation, LoRaProfile, LoRaProfileConfig, LoRaProfileError,
...
use reticulum_radio_lora_phy::{
    IrqTimestampCapture, NoopTxHooks, Sx126xRadioError, Sx126xRadioOperation, Sx126xRnodeRadio,
    Sx126xRnodeSettings,
};
...
/// Requested SX1262 output of the default configuration.
///
...
pub const E290_NA915_DEFAULT_REQUESTED_POWER_DBM: i32 = 14;

/// Raw `SetTxParams` power selected by the current Semtech optimal +14 dBm row.
pub const E290_NA915_DEFAULT_RAW_SET_TX_PARAMS_DBM: i8 = 22;
...
impl E290Na915TxPower {
    ...
    const fn fingerprint(self) -> RadioConfigurationFingerprint {
        match self {
            Self::Dbm14 => E290_NA915_DEFAULT_CONFIGURATION_FINGERPRINT,
            Self::Dbm17 => E290_NA915_DEFAULT_17_DBM_CONFIGURATION_FINGERPRINT,
            Self::Dbm20 => E290_NA915_DEFAULT_20_DBM_CONFIGURATION_FINGERPRINT,
            Self::Dbm22 => E290_NA915_DEFAULT_22_DBM_CONFIGURATION_FINGERPRINT,
        }
    }
}
...
///
/// Powered two-node checkpoints observed healthy receive operations exceeding
/// 1.5 seconds when flash and protocol work delayed the cooperative executor.
/// The outer watchdog is destructive by contract, so a scheduling overrun must
/// not be mistaken for a wedged SX1262 operation.
...
pub const E290_NA915_DEFAULT_CONFIGURATION_FINGERPRINT: RadioConfigurationFingerprint =
...
pub const E290_NA915_DEFAULT_17_DBM_CONFIGURATION_FINGERPRINT: RadioConfigurationFingerprint =
    RadioConfigurationFingerprint::new([
        0x45, 0x32, 0x39, 0x30, 0x4e, 0x41, 0x39, 0x31, 0x35, 0x44, 0x45, 0x56, 0x30, 0x30, 0x30,
        0x32,
    ]);
...
pub const E290_NA915_DEFAULT_20_DBM_CONFIGURATION_FINGERPRINT: RadioConfigurationFingerprint =
    RadioConfigurationFingerprint::new([
        0x45, 0x32, 0x39, 0x30, 0x4e, 0x41, 0x39, 0x31, 0x35, 0x44, 0x45, 0x56, 0x30, 0x30, 0x30,
        0x33,
    ]);
...
pub const E290_NA915_DEFAULT_22_DBM_CONFIGURATION_FINGERPRINT: RadioConfigurationFingerprint =
    RadioConfigurationFingerprint::new([
        0x45, 0x32, 0x39, 0x30, 0x4e, 0x41, 0x39, 0x31, 0x35, 0x44, 0x45, 0x56, 0x30, 0x30, 0x30,
        0x34,
    ]);
...
pub const E290_NA915_DEFAULT_PROFILE: LoRaProfile = match validate_lora_profile(LoRaProfileConfig {
...
pub enum E290RadioConfigurationError {
    ...
    InvalidProfile(LoRaProfileError),
}

impl E290RadioConfigurationError {
    /// Underlying board/profile validation error.
    pub const fn profile_error(self) -> LoRaProfileError {
        match self {
...
impl E290RadioConfiguration {
    ...
    pub const fn na915_default(tx_power: E290Na915TxPower) -> Self {
        Self {
            id: tx_power.configuration_id(),
            profile: E290_NA915_DEFAULT_PROFILE,
    ...
    pub const fn try_from_profile(
        ...
        let profile = match validate_lora_profile(LoRaProfileConfig {
        ...
        if profile.frequency_hz() == E290_NA915_DEFAULT_PROFILE.frequency_hz()
            && profile.bandwidth().hz() == E290_NA915_DEFAULT_PROFILE.bandwidth().hz()
            && profile.spreading_factor().factor()
                == E290_NA915_DEFAULT_PROFILE.spreading_factor().factor()
            && profile.coding_rate().denom() == E290_NA915_DEFAULT_PROFILE.coding_rate().denom()
        {
            return Ok(Self::na915_default(tx_power));
        }
...
const fn maximum_receive_operation_us(profile: LoRaProfile) -> NonZeroU64 {
...
/// Immutable default E290 NA915 configuration.
pub const E290_NA915_DEFAULT_CONFIGURATION: E290RadioConfiguration =
    E290RadioConfiguration::na915_default(E290Na915TxPower::Dbm14);

/// Immutable E290 NA915 configuration at requested +17 dBm.
pub const E290_NA915_DEFAULT_17_DBM_CONFIGURATION: E290RadioConfiguration =
    E290RadioConfiguration::na915_default(E290Na915TxPower::Dbm17);

/// Immutable E290 NA915 configuration at requested +20 dBm.
pub const E290_NA915_DEFAULT_20_DBM_CONFIGURATION: E290RadioConfiguration =
    E290RadioConfiguration::na915_default(E290Na915TxPower::Dbm20);

/// Immutable E290 NA915 configuration at requested +22 dBm.
pub const E290_NA915_DEFAULT_22_DBM_CONFIGURATION: E290RadioConfiguration =
    E290RadioConfiguration::na915_default(E290Na915TxPower::Dbm22);
...
mod tests {
    ...
    fn build_radio_with_configuration(configuration: E290RadioConfiguration) -> TestRig {
        ...
        let timestamps = Box::leak(Box::new(IrqTimestampCapture::new(stepped_us)));
    ...
    fn build_radio() -> TestRig {
        build_radio_with_configuration(E290_NA915_DEFAULT_CONFIGURATION)
    }
    ...
    fn every_boot_power_builds_a_distinct_immutable_configuration() {
        let cases = [
            (
                ...
                E290_NA915_DEFAULT_CONFIGURATION,
            ),
            (
                E290Na915TxPower::Dbm17,
                E290RadioConfigurationId::Na915DevRequested17Dbm,
                E290_NA915_DEFAULT_17_DBM_CONFIGURATION_FINGERPRINT,
                E290_NA915_DEFAULT_17_DBM_CONFIGURATION,
            ),
            (
                E290Na915TxPower::Dbm20,
                E290RadioConfigurationId::Na915DevRequested20Dbm,
                E290_NA915_DEFAULT_20_DBM_CONFIGURATION_FINGERPRINT,
                E290_NA915_DEFAULT_20_DBM_CONFIGURATION,
            ),
            (
                E290Na915TxPower::Dbm22,
                E290RadioConfigurationId::Na915DevRequested22Dbm,
                E290_NA915_DEFAULT_22_DBM_CONFIGURATION_FINGERPRINT,
                E290_NA915_DEFAULT_22_DBM_CONFIGURATION,
            ),
        ];

        for (power, id, fingerprint, constant_configuration) in cases {
            let constructed = E290RadioConfiguration::na915_default(power);
    ...
    fn configurable_constructor_preserves_the_compatibility_profile() {
        for power in [
            ...
            assert_eq!(
                ...
                Ok(E290RadioConfiguration::na915_default(power))
            );
        }
    }
    ...
    fn configurable_constructor_validates_the_fitted_path_and_modulation() {
        ...
        assert!(matches!(
            ...
            LoRaProfileError::ChannelOutsideSupportedRange { .. }
        ));
        ...
        assert_eq!(
            ...
            LoRaProfileError::UnverifiedRnodeLdroCombination {
                spreading_factor: 11,
                bandwidth_hz: 125_000,
            }
        );
    ...
    fn receive_watchdog_scales_with_the_configured_symbol_and_frame_times() {
        ...
        assert!(
            slow.maximum_receive_operation_us()
                > E290_NA915_DEFAULT_CONFIGURATION.maximum_receive_operation_us()
        );
    }

    #[test]
    fn opaque_configuration_is_exact_and_watchdog_is_conservative() {
        let configuration = E290_NA915_DEFAULT_CONFIGURATION;
        ...
        assert_eq!(E290_NA915_DEFAULT_RAW_SET_TX_PARAMS_DBM, 22);
    ...
    fn every_boot_power_repeats_its_exact_pa_sequence_for_both_physical_frames() {
        for (power, pa_command) in [
            ...
            let configuration = E290RadioConfiguration::na915_default(power);
            let TestRig {
                mut radio,
                controls,
                reset,
