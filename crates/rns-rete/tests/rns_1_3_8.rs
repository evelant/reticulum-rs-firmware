//! Interoperability checks against the released Python RNS 1.3.8 behavior.
...
use reticulum_rns_rete::{
    ApplicationEvent, ApplicationLinkRole, ConformanceNode, DestHash, DestType, DestinationType,
    Direction, EmbeddedNodeConfig, Identity, IngressDisposition, InterfaceId, LinkId, LinkState,
    MonotonicInstant, OutboundDispatchInterval, PacketType, RNS_MTU, TxTarget,
...
fn verify_released_vectors() -> Result<usize, String> {
    ...
    let mut actual_announce = [0u8; RNS_MTU];
    ...
    require(
        corpus.plain_data.aspects == ["interop"],
    ...
    let mut outbound_bytes = [0_u8; RNS_MTU];
    let outbound = sender
        .prepare_data_into(
            &announce_dest_hash,
            b"rete live path",
    ...
    let outbound_bytes = &outbound_bytes[..usize::from(outbound.packet_len())];
    let outbound_packet = parse_packet(outbound_bytes)
    ...
    let forwarded = relay.ingest(outbound_bytes, 1_700_000_004, InterfaceId(2), &mut ZeroRng);
    require(
    ...
    // H2 back to H1.
    let mut python_endpoint = ConformanceNode::new(
        identity,
        &corpus.announce.app_name,
        &aspects,
    ...
    checks += verify_lrrtt_rete_lifecycle()?;
    checks += verify_three_node_relayed_link()?;

    Ok(checks)
}

struct LinkHandshakeFixture {
    initiator: ConformanceNode,
    responder: ConformanceNode,
    link_id: LinkId,
...
fn verify_lrrtt_rete_lifecycle() -> Result<usize, String> {
    let mut checks = 0;
    ...
    let mut stale_repeat = prepare_link(
        "stale LRRTT repeat",
        0xd1,
        0xd2,
        51,
    ...
    activate_responder(&mut stale_repeat, 250_000)?;
    ...
    for (label, state, initiator_tag, responder_tag, interface, base_micros) in [
        ...
        if !matches!(state, ResponderState::Handshake) {
            activate_responder(&mut fixture, 250_000)?;
        }
        if matches!(state, ResponderState::Stale) {
        ...
        let expected_state = match state {
            ResponderState::Handshake => LinkState::Handshake,
            ResponderState::Active => LinkState::Active,
            ResponderState::Stale => LinkState::Stale,
        };
        require(
        ...
        let expected_failed =
            before.links_failed + u64::from(matches!(state, ResponderState::Handshake));
        require(
    ...
    let mut corrupt_stale = prepare_link(
    ...
    activate_responder(&mut corrupt_stale, 250_000)?;
    ...
    *last_byte ^= 0x01;
    ...
    let mut zero = prepare_link("zero RTT", 0xdb, 0xdc, 61, 1_700_060_000_000_001, 0)?;
    activate_responder(&mut zero, 0)?;
...
fn prepare_link(
...
) -> Result<LinkHandshakeFixture, String> {
    ...
    initiator
        .register_peer(&responder_identity, "reticulum", &["interop"], now)
...
fn activate_responder(
    fixture: &mut LinkHandshakeFixture,
    ...
    require(
        ...
        "Rete responder did not establish from its first LRRTT",
    )
}
...
#[test]
fn rns_1_3_8_interoperability() {
    assert_eq!(reticulum_radio_interface::RNS_MTU, RNS_MTU);
