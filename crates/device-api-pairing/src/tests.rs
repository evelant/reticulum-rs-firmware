...
const BEGIN_RESPONSE_WIRE: &str = "";
const PROOF_REQUEST_WIRE: &str = "0007524441310126010101010101010101010101010101010102010101010101010240020101010201020139101112131415161718191a1b1c1d1e1f0807060504030201404142434445464748494a4b4c4d4e4f505152535455565758595a5b5c5d5e5f0101010101010101010101010101010100";
const PROOF_RESPONSE_WIRE: &str = "";
const ACTIVATE_REQUEST_WIRE: &str = "000752444131012801010101010101010101010101010101010202010101010101023839101112131415161718191a1b1c1d1e1f0807060504030201b8d1cd5617191b45c59c1e8ca75ae9d317e36f94be7740c3fb9da1467cd2abba0101010101010101010101010101010100";
const ACTIVATE_RESPONSE_WIRE: &str = "0007524441310129010101010101010101010101010101010102020101010101010240010101010101010139101112131415161718191a1b1c1d1e1f0807060504030201a7d3810f6f0a30b71cb614c164eee53705f0b6d13170c2b419b6f9b1bc4568030101010101010101010101010101010100";
...
const TRANSCRIPT_HASH: [u8; 32] = [
    0xbf, 0xfd, 0xaf, 0x52, 0xb5, 0x5f, 0xe9, 0x3a, 0xfc, 0x49, 0xc8, 0x93, 0xe1, 0x6d, 0x2c, 0x82,
    0x94, 0x3e, 0x04, 0x2c, 0xf0, 0x08, 0x7f, 0xb6, 0x6f, 0x6d, 0xf9, 0x9e, 0x33, 0x98, 0xcf, 0x6a,
];
const CLIENT_PROOF: [u8; 32] = [
    0xb8, 0xd1, 0xcd, 0x56, 0x17, 0x19, 0x1b, 0x45, 0xc5, 0x9c, 0x1e, 0x8c, 0xa7, 0x5a, 0xe9, 0xd3,
    0x17, 0xe3, 0x6f, 0x94, 0xbe, 0x77, 0x40, 0xc3, 0xfb, 0x9d, 0xa1, 0x46, 0x7c, 0xd2, 0xab, 0xba,
];
const ACTIVATION_CONFIRMATION: [u8; 32] = [
    0xa7, 0xd3, 0x81, 0x0f, 0x6f, 0x0a, 0x30, 0xb7, 0x1c, 0xb6, 0x14, 0xc1, 0x64, 0xee, 0xe5, 0x37,
    0x05, 0xf0, 0xb6, 0xd1, 0x31, 0x70, 0xc2, 0xb4, 0x19, 0xb6, 0xf9, 0xb1, 0xbc, 0x45, 0x68, 0x03,
];
...
fn proof_request() -> ProofStartRequest {
    ProofStartRequest::new(1, credential_id(), generation(), CLIENT_NONCE).unwrap()
}
...
fn independent_record_vectors_match_every_successful_flight() {
    ...
    assert!(matches!(
        PairingRequest::from_record(decode_wire(&hex::decode(BEGIN_REQUEST_WIRE).unwrap())),
    ...
    let decoded =
        PairingResponse::from_record(decode_wire(&hex::decode(BEGIN_RESPONSE_WIRE).unwrap()))
            .unwrap();
    match decoded {
    ...
    let decoded =
        PairingRequest::from_record(decode_wire(&hex::decode(PROOF_REQUEST_WIRE).unwrap()))
            .unwrap();
    match decoded {
    ...
    let decoded =
        PairingResponse::from_record(decode_wire(&hex::decode(PROOF_RESPONSE_WIRE).unwrap()))
            .unwrap();
    match decoded {
    ...
    let decoded =
        PairingRequest::from_record(decode_wire(&hex::decode(ACTIVATE_REQUEST_WIRE).unwrap()))
            .unwrap();
    match decoded {
    ...
    let decoded =
        PairingResponse::from_record(decode_wire(&hex::decode(ACTIVATE_RESPONSE_WIRE).unwrap()))
            .unwrap();
    match decoded {
    ...
    assert!(matches!(
        PairingRequest::from_record(decode_wire(&hex::decode(ABORT_REQUEST_WIRE).unwrap())),
    ...
    assert!(matches!(
        PairingResponse::from_record(decode_wire(&hex::decode(ABORT_RESPONSE_WIRE).unwrap())),
...
fn independent_crypto_vectors_match_and_verify_both_directions() {
    ...
    let expected_confirmation = crate::ActivationConfirmation::calculate(&psk, &transcript, &proof);
    ...
    let verified = ClientProof::from_bytes(CLIENT_PROOF)
        ...
        .unwrap();
    let server_confirmation = verified.into_activation_confirmation(&psk);
    ...
    assert!(
        !crate::ActivationConfirmation::from_bytes(credential_id(), generation(), bad_confirmation)
            .unwrap()
...
fn every_coarse_failure_round_trips_with_echoed_arbitrary_sequences() {
    ...
    for (index, failure) in pairing.into_iter().enumerate() {
        ...
        match PairingResponse::from_record(decode_wire(
            FramedRecord::encode(&begin).unwrap().encoded(),
        ))
        .unwrap()
        {
            PairingResponse::Begin(response) => {
        ...
        match PairingResponse::from_record(decode_wire(
            FramedRecord::encode(&proof).unwrap().encoded(),
        ))
        .unwrap()
        {
            PairingResponse::ProofStart(response) => {
    ...
    for code in 0_u8..=4 {
        ...
        assert!(matches!(
            PairingResponse::from_record(decode_wire(
                FramedRecord::encode(&record).unwrap().encoded()
            )),
            Ok(PairingResponse::AbortCurrent(response))
...
fn request_union_rejects_nonzero_pre_authentication_fields_kinds_and_lengths() {
    ...
    for (record, expected) in cases {
        assert_eq!(PairingRequest::from_record(record).err(), Some(expected));
    }
}
...
fn response_union_rejects_nonzero_pre_authentication_fields_shapes_and_codes() {
    ...
    for (record, expected) in cases {
        assert_eq!(PairingResponse::from_record(record).err(), Some(expected));
    }
    ...
    for kind in [
        ...
        for code in 5_u8..=u8::MAX {
            assert_eq!(
                PairingResponse::from_record(record(kind, [0; 16], 0, &[code], [0; 16])).err(),
...
fn profile_reserved_and_every_required_nonzero_field_are_validated() {
    ...
    let profile_cases = [
        ...
        (4, 2, DecodeError::UnsupportedSuite { observed: 2 }),
    ...
    for (index, value, expected) in profile_cases {
        ...
        assert_eq!(
            PairingRequest::from_record(record(
                RECORD_KIND_PROOF_START_REQUEST,
                [0; 16],
                1,
                &request_payload,
    ...
    let mut begin =
        BeginOffer::after_pending_commit(device_id(), credential_id(), generation(), psk())
            .unwrap()
            .encode_payload();
    for index in 1..8 {
        begin[index] = 1;
        assert_eq!(
            PairingResponse::from_record(record(
                RECORD_KIND_BEGIN_RESPONSE,
                [0; 16],
                0,
                &begin[..],
...
fn activation_confirmation_binds_transcript_and_every_client_proof_byte() {
    ...
    for index in 0..ACTIVATION_CONFIRMATION.len() {
        ...
        changed[index] ^= 1;
        assert!(
            !crate::ActivationConfirmation::from_bytes(credential_id(), generation(), changed)
                .unwrap()
                .verify(&psk, &transcript, &proof)
        );
    }
...
fn typed_constructors_reject_zero_values_and_transcript_reference_mismatch() {
    ...
    assert_eq!(
        ProofStartRequest::new(0, CredentialId::new([0; 16]), generation(), CLIENT_NONCE),
    ...
    assert_eq!(
        ProofStartRequest::new(0, credential_id(), generation(), [0; 32]),
        Err(InvalidField::ZeroClientNonce)
    );

    let other =
        ProofStartRequest::new(1, CredentialId::new([0xaa; 16]), generation(), CLIENT_NONCE)
            .unwrap();
...
fn every_success_response_rejects_noncanonical_profile_reference_and_shape() {
    ...
    let begin = BeginOffer::after_pending_commit(device_id(), credential_id(), generation(), psk())
        .unwrap()
        .encode_payload();
    assert_eq!(
        PairingResponse::from_record(record(
...
fn activate_continuation_and_confirmation_reject_substituted_references() {
    ...
    let confirmation = canonical
        ...
        .unwrap()
        .into_activation_confirmation(&psk);
    ...
    let substituted_id = PairingResponse::from_record(record(
        RECORD_KIND_ACTIVATE_RESPONSE,
        [0; 16],
        2,
        &payload,
    ...
    payload[24..32].copy_from_slice(&(GENERATION + 1).to_le_bytes());
    let substituted_generation = PairingResponse::from_record(record(
        RECORD_KIND_ACTIVATE_RESPONSE,
        [0; 16],
        2,
        &payload,
