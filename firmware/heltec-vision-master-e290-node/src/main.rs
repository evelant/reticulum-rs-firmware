...
//! During alpha development every production profile emits lifecycle and fault
//! logs over USB Serial/JTAG; wireless profiles still do not expose a second
//! RDA1 bearer there. The node
...
#[cfg_attr(
    ...
    allow(
        dead_code,
        unused_imports,
        reason = "wireless profiles retain USB for logs but not the framed API owner"
    )
)]
...
use reticulum_board_heltec_vision_master_e290_radio::{
    E290Na915TxPower, E290Radio, E290RadioConfiguration,
};
...
#[cfg(feature = "ble-api-proof")]
use reticulum_heltec_vision_master_e290_node::ble_api_profile::{
...
#[cfg(feature = "wifi-station-proof")]
use reticulum_heltec_vision_master_e290_node::wifi_station_profile::WifiStationStatusCell;
#[cfg(feature = "wifi-tcp-proof")]
...
async fn product_main(spawner: Spawner, usb_boot_boundary: ProductUsbBootBoundary) -> ! {
    ...
    // The legacy wired profile owns this same byte stream as its framed RDA1
    // bearer. Text logging there would corrupt the protocol, so production USB
    // logs are enabled on the wireless appliance profiles only.
    #[cfg(any(feature = "ble-api-proof", feature = "wifi-api-proof"))]
    ...
    #[cfg(feature = "runtime-measurement-hil")]
    let runtime_measurement_started_us;

    {
        ...
        #[cfg(feature = "runtime-measurement-hil")]
        {
            runtime_measurement_started_us = monotonic_us();
        }
        #[cfg(feature = "runtime-measurement-hil")]
        {
        ...
        #[cfg(any(feature = "wifi-api-proof", feature = "wifi-station-proof"))]
        ...
        #[cfg(all(feature = "display", feature = "ble-api-proof"))]
        ...
        // Finish the qualified boot-composition Home render after the physical
...
fn inert_before_rtos() -> ! {
    ...
    // error is visible through the production USB logger; the separately named
    // startup diagnostic additionally emits a compact stage marker.
    #[cfg(reticulum_e290_ble_startup_diagnostic)]
    esp_println::println!(
