...
use reticulum_board_heltec_vision_master_e290_radio::{E290Na915TxPower, E290RadioConfiguration};
...
use reticulum_device_api::{
    ...
    LxmfDiscoveredPeer as ApiLxmfDiscoveredPeer,
    LxmfIngressObservation as ApiLxmfIngressObservation, LxmfIngressSignal as ApiLxmfIngressSignal,
...
use reticulum_device_api_adapter::{
    ...
    ManualServiceAnnouncePort, NetworkConfigPort, NetworkConfigPortError, NodeDiagnosticsPort,
    NodeDiagnosticsPortError, NomadFetchPort, PeerDiscoveryPort, PeerDiscoveryPortError,
    ReticulumProbePort, SubmissionAcceptance, SubmissionPort, SubmissionPortError,
};
...
use reticulum_heltec_vision_master_e290_node::announce_time::{
...
use reticulum_heltec_vision_master_e290_node::partition_contract::{
    ...
    LXMF_STORE_LABEL_BYTES, LXMF_STORE_LEN, LXMF_STORE_OFFSET, MESSAGE_STORE_LABEL_BYTES,
    MESSAGE_STORE_LEN, MESSAGE_STORE_OFFSET, NETWORK_CONFIG_LEN, NETWORK_CONFIG_OFFSET,
    NODE_IDENTITY_LABEL_BYTES, NODE_IDENTITY_LEN, NODE_IDENTITY_OFFSET, NODE_JOURNAL_LABEL_BYTES,
    NODE_JOURNAL_LEN, NODE_JOURNAL_OFFSET, REQUIRED_FLASH_BYTES, UNDEFINED_DATA_SUBTYPE,
};
#[cfg(feature = "runtime-measurement-hil")]
use reticulum_heltec_vision_master_e290_node::runtime_measurement::RETICULUM_RUNTIME_PROOF_TRACE_EVIDENCE;
#[cfg(feature = "wifi-tcp-proof")]
use reticulum_heltec_vision_master_e290_node::wifi_tcp_profile::{
...
use reticulum_heltec_vision_master_e290_node::{
    ...
    network_config_binding, node_journal_binding, rns_inbox_binding,
};
...
use reticulum_lxmf_mailbox_store::{
    ...
    MailboxStoreMount, MountedMailboxStore, acknowledge_through as acknowledge_mailbox_through,
    mount as mount_mailbox_store, provision as provision_mailbox_store,
    rebaseline as rebaseline_mailbox_store,
};
...
use reticulum_heltec_vision_master_e290_node::diagnostics_api::{
...
use reticulum_heltec_vision_master_e290_node::network_config_replay::NetworkConfigMutationReceipt;
use reticulum_heltec_vision_master_e290_node::radio_diagnostics::{
    RadioDiagnosticsCell, RadioRuntimeState,
};
use reticulum_heltec_vision_master_e290_node::radio_trace::RadioTraceCursor;
#[cfg(feature = "wifi-station-proof")]
use reticulum_heltec_vision_master_e290_node::wifi_station_profile::{
    WifiStationPhase, WifiStationStatusCell,
};
...
impl ProductFlashOwner {
    ...
    pub(crate) fn into_storage_coordinator(
        ...
        #[cfg(feature = "wifi-station-proof")] wifi_station_status: &'static WifiStationStatusCell,
        #[cfg(feature = "wifi-tcp-proof")] wifi_tcp_status: &'static WifiTcpStatusCell,
    ) -> ProductStorageCoordinator {
        let lxmf_mailbox = lxmf.as_ref().and_then(|store| {
            mount_or_baseline_lxmf_mailbox(&mut *self.flash, self.lxmf_mailbox_binding, store).ok()
        });
        ...
        // A fresh notification store is durably baselined to the pre-existing
        // LXMF tail before ingress opens. If that cannot be established, fail
        // this service closed rather than classifying later messages against
        // an unstable migration watermark.
...
fn mount_or_baseline_lxmf_mailbox(
    ...
    let baseline = lxmf.latest_receipt().map(mailbox_cursor_from_lxmf_receipt);
    match mount_mailbox_store(&mut access)? {
        MailboxStoreMount::Mounted(mounted) => {
            if mailbox_cursor_matches_lxmf(lxmf, mounted.acknowledged_cursor()) {
                ...
                match rebaseline_mailbox_store(mounted, &mut access, baseline) {
                    Ok(rebaselined) => Ok(rebaselined),
                    Err(error) => match mount_mailbox_store(&mut access) {
                        Ok(MailboxStoreMount::Mounted(reconciled))
                            if reconciled.acknowledged_cursor() == baseline =>
                        {
                            Ok(reconciled)
                        }
                        _ => Err(error),
        ...
        MailboxStoreMount::Unprovisioned => match provision_mailbox_store(&mut access, baseline) {
            Ok(mounted) => Ok(mounted),
            Err(error) => match mount_mailbox_store(&mut access) {
                Ok(MailboxStoreMount::Mounted(mounted))
                    if mounted.acknowledged_cursor() == baseline =>
                {
                    Ok(mounted)
                }
                _ => Err(error),
...
impl ProductStorageCoordinator {
    ...
    pub(crate) fn wifi_station_boot_plan(
        &self,
    ) -> Option<reticulum_heltec_vision_master_e290_node::wifi_station_profile::WifiStationBootPlan>
    {
        match &self.network_config {
            ProductNetworkConfigState::Mounted(mounted) => {
                Some(reticulum_heltec_vision_master_e290_node::wifi_station_profile::WifiStationBootPlan::select(
                    mounted.revision().generation().get(),
                    mounted.configuration(),
                ))
            }
            ProductNetworkConfigState::FreshErased { .. } => {
                Some(reticulum_heltec_vision_master_e290_node::wifi_station_profile::WifiStationBootPlan::Disabled {
                    applied_revision: 0,
                })
            }
...
impl ProductAuthenticatedApiPort<'_> {
    ...
    fn network_config_applied_revision(&self) -> u64 {
        ...
        #[cfg(all(feature = "wifi-station-proof", not(feature = "wifi-tcp-proof")))]
        {
            let station_revision = self.wifi_station_status.snapshot().applied_revision;
            // The station-only slice has not applied an enabled TCP peer. Keep
            // desired/applied revisions different until interface 2 exists.
            if self
                .network_config
                .redacted()
                .tcp_peer()
    ...
    fn next(
        ...
        for receipt in store.receipts() {
            ...
            let ingress = metadata.ingress_observation().map(|ingress| {
                ApiLxmfIngressObservation::new(
                    ingress.interface().get(),
                    ingress.signal().map(|signal| {
                        ApiLxmfIngressSignal::new(signal.rssi_dbm(), signal.snr_db())
                    }),
                )
            });
