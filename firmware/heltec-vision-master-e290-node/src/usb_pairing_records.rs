//! Shared pre-authentication request demultiplexing for the E290 USB stream.
//!
...
/// Exact request family and operation decoded from the shared USB stream.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum UsbPreAuthenticationRequestKind {
...
impl UsbPreAuthenticationRequestKind {
...
#[must_use = "a decoded USB request must be sequenced, transferred, or explicitly dropped"]
pub enum UsbPreAuthenticationRequest {
...
impl UsbPreAuthenticationRequest {
    ...
    pub const fn kind(&self) -> UsbPreAuthenticationRequestKind {
        match self {
            Self::Control(ControlRequest::Status { .. }) => UsbPreAuthenticationRequestKind::Status,
            Self::Control(ControlRequest::Initialize { .. }) => {
                UsbPreAuthenticationRequestKind::Initialize
            }
            Self::Live(PairingRequest::Begin(_)) => UsbPreAuthenticationRequestKind::Begin,
            Self::Live(PairingRequest::ProofStart(_)) => {
                UsbPreAuthenticationRequestKind::ProofStart
            }
            Self::Live(PairingRequest::Activate(_)) => UsbPreAuthenticationRequestKind::Activate,
            Self::Live(PairingRequest::AbortCurrent(_)) => {
                UsbPreAuthenticationRequestKind::AbortCurrent
            }
        }
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum UsbPreAuthenticationDecodeError {
...
/// Decode exactly the six request kinds accepted on the shared USB stream.
///
/// Response and unknown kinds are rejected before either typed codec consumes
/// the record. The caller must advance its shared exact-next sequence gate only
/// after this function succeeds and it can retain the returned owner.
    ...
    match kind {
        RECORD_KIND_STATUS_REQUEST | RECORD_KIND_INITIALIZE_REQUEST => {
            ControlRequest::from_record(record)
                .map(UsbPreAuthenticationRequest::Control)
                .map_err(|_| UsbPreAuthenticationDecodeError::InvalidControl)
        }
...
mod tests {
    ...
    fn decode_and_accept(record: Record, gate: &mut ExactNextSequenceGate) {
        let request = decode_usb_pre_authentication_request(record)
    ...
    fn duplicate_and_gap_rejection_cross_protocol_families() {
        ...
        let duplicate = decode_usb_pre_authentication_request(
            PairingRequest::Begin(BeginRequest::new(0)).into_record(),
        )
        ...
        let gap = decode_usb_pre_authentication_request(ControlRequest::status(2).into_record())
    ...
    fn rejected_records_do_not_advance_the_callers_sequence_gate() {
        ...
        assert!(matches!(
            decode_usb_pre_authentication_request(response),
            Err(UsbPreAuthenticationDecodeError::UnsupportedKind(_))
        ));
        ...
        assert_eq!(
            decode_usb_pre_authentication_request(malformed).err(),
        ...
        assert_eq!(
            decode_usb_pre_authentication_request(unknown).err(),
            Some(UsbPreAuthenticationDecodeError::UnsupportedKind(0xfe))
        );
        ...
        assert!(matches!(
            decode_usb_pre_authentication_request(pairing_response),
            Err(UsbPreAuthenticationDecodeError::UnsupportedKind(_))
        ));
    ...
    fn activate_decode_retains_the_only_proof_owner_and_exact_kind() {
        ...
        let decoded =
            decode_usb_pre_authentication_request(PairingRequest::Activate(request).into_record())
                .expect("activate record decodes");
        assert_eq!(decoded.kind(), UsbPreAuthenticationRequestKind::Activate);
        match decoded {
            UsbPreAuthenticationRequest::Live(PairingRequest::Activate(request)) => {
