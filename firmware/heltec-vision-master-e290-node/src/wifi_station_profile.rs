//! Bounded product model for the additive Wi-Fi station proof.
//!
...
/// Upstream-tested esp-radio receive-queue depth.
///
/// The earlier low-throughput experiment reduced this to three. Powered use
/// later lost ordinary LAN unicast while association and DHCP state remained
/// live, so the border-node profile keeps the pinned driver's robust baseline.
...
///
/// The earlier 72 KiB internal-heap profile reduced this to four so controller
/// startup could fit. Wi-Fi station builds now provide 120 KiB of strict
/// internal heap, restoring the upstream receive pool for burst tolerance and
/// BLE coexistence qualification.
