...
//! local API bearer, so [`ConnectionId`] is unambiguous. A later concurrent
//! bearer must not clone this singleton topology with an independent connection/epoch
...
///
/// ```compile_fail
/// use reticulum_e290_firmware::session_admission_handoff::SessionAdmissionOutcome;
...
///
/// The handoff accepts any future device-API bearer adapter; credential
/// ownership and selection policy remain on the node side.
