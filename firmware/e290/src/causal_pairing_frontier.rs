...
//!
//! The local API bearer records event time before transferring commands through
//! two owning handoffs. This selector restores that causal order at the sole
//! node owner without depending on executor scheduling or channel polling order.
...
/// sole bearer producer can transfer a live or session-admission request and
/// then observe its disconnect in the same millisecond; admitting the request first
