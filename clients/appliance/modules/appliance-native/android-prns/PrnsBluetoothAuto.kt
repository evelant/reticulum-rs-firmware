...
internal class PrnsBluetoothAuto(private val context: Context) {
    private val bluetoothManager =
        context.getSystemService(Context.BLUETOOTH_SERVICE) as? BluetoothManager
    private val adapter: BluetoothAdapter? = bluetoothManager?.adapter
    private val peerCapacity = PrnsNativeBridge.nativeBlePeerCapacity().coerceAtLeast(1)
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            direct.put(octets)
            PrnsNativeBridge.nativeBleSighting(direct, result.rssi)
        }
    ...
    private val gattServerCallback = object : BluetoothGattServerCallback() {
        ...
        override fun onCharacteristicWriteRequest(
            ...
            val admission = when (characteristic.uuid) {
                ...
                } ?: PrnsNativeBridge.BLE_INGRESS_CLOSED
            ...
            if (responseNeeded) {
                val status = when (admission) {
                    PrnsNativeBridge.BLE_INGRESS_ACCEPTED -> BluetoothGatt.GATT_SUCCESS
                    PrnsNativeBridge.BLE_INGRESS_FULL -> ATT_INSUFFICIENT_RESOURCES
                    else -> BluetoothGatt.GATT_FAILURE
                }
                gattServer?.sendResponse(device, requestId, status, offset, null)
            }
            if (admission == PrnsNativeBridge.BLE_INGRESS_CLOSED) {
            ...
            if (descriptor.characteristic.uuid == COLUMBA_TX) {
                ...
                if (links.size >= peerCapacity) {
                    ...
                    if (octets == null) {
                        ...
                        val direct = ByteBuffer.allocateDirect(6)
                        direct.put(octets)
                        if (!PrnsNativeBridge.nativeBleLinkUp(connId, direct, RSSI_NONE, false)) {
        ...
        override fun onNotificationSent(device: BluetoothDevice, status: Int) {
            ...
            if (link.completeGattOperation(GattOperationKind.ServerNotify)) {
                PrnsNativeBridge.nativeBleWakePumps()
    ...
    private fun handleColumbaRxWrite(device: BluetoothDevice, value: ByteArray): Int {
        ...
        if (existingConnId == null) {
            if (value.size != COLUMBA_IDENTITY_LEN) {
                ...
                return PrnsNativeBridge.BLE_INGRESS_CLOSED
            }
            val octets = parseMac(address) ?: return PrnsNativeBridge.BLE_INGRESS_CLOSED
            val connId = nextConnId.getAndIncrement()
            ...
            val admitted = PrnsNativeBridge.nativeBleColumbaLinkUp(
                connId,
            ...
            return PrnsNativeBridge.BLE_INGRESS_ACCEPTED
        }
    ...
    private fun deliverGattInbound(connId: Int, dataLane: Boolean, value: ByteArray): Int {
        ...
        val admission =
            if (dataLane) {
                PrnsNativeBridge.nativeBleDataIn(connId, direct, value.size)
            } else {
                PrnsNativeBridge.nativeBleControlIn(connId, direct, value.size)
            }
        if (admission != PrnsNativeBridge.BLE_INGRESS_ACCEPTED) {
            val reason = if (admission == PrnsNativeBridge.BLE_INGRESS_FULL) "full" else "closed"
    ...
    private fun localBleIdentity(): ByteArray? {
        ...
        val n = PrnsNativeBridge.nativeBleIdentity(direct)
    ...
    fun start() {
        ...
        } else if (!runCatching { adapter.isEnabled }.getOrDefault(false)) {
    ...
    private fun startOwnedWorker(
        ...
        worker.name = "AppliancePrnsBle-$name"
        workers.add(worker)
        if (radioScoped) {
    ...
    private fun startRadioStatePump() {
        startWorker("radio-state") {
            ...
            var generation = PrnsNativeBridge.nativeBleWorkGeneration()
            while (running) {
                val state = PrnsNativeBridge.nativeBleDesiredState()
                val wantsRadio = (state and PrnsNativeBridge.BLE_RADIO_ENABLED) != 0
                ...
                generation = PrnsNativeBridge.nativeBleWaitForWork(generation, RADIO_STATE_RETRY_MS)
            }
    ...
    private fun applyDesiredRadioState(state: Int) {
        val wantRadio = (state and PrnsNativeBridge.BLE_RADIO_ENABLED) != 0
        advertisingWanted = (state and PrnsNativeBridge.BLE_RADIO_ADVERTISING) != 0
        scanningWanted = (state and PrnsNativeBridge.BLE_RADIO_SCANNING) != 0
    ...
    private fun startL2capListener(adapter: BluetoothAdapter): Boolean {
        ...
        PrnsNativeBridge.nativeBleSetPsm(psm)
        startRadioWorker("l2cap-listener") {
            while (running && radioActive) {
                ...
                PrnsNativeBridge.nativeBleL2capUp(connId)
    ...
    private fun startL2capPumps(connId: Int, socket: BluetoothSocket) {
        startLinkWorker(connId, "l2cap-in-$connId") {
            ...
            while (running && radioActive && links.containsKey(connId)) {
                ...
                if (n > 0) {
                    direct.clear()
                    direct.put(buf, 0, n)
                    if (!PrnsNativeBridge.nativeBleL2capIn(connId, direct, n)) {
        ...
        startLinkWorker(connId, "l2cap-out-$connId") {
            ...
            while (running && radioActive && links.containsKey(connId)) {
                direct.clear()
                val n = PrnsNativeBridge.nativeBleL2capOut(connId, direct)
                if (n > 0) {
                    direct.position(0)
                    ...
                    generation = PrnsNativeBridge.nativeBleWaitForWork(generation, 0)
                }
            }
        }
    }
        ...
        startWorker("control-out") {
            ...
            while (running) {
                ...
                for (link in links.values) {
                    direct.clear()
                    val n = PrnsNativeBridge.nativeBleControlOut(link.connId, direct)
                    if (n > 0) {
                        pending = true
                        direct.position(0)
                        direct.get(scratch, 0, n)
                        when (deliverControl(link, scratch.copyOf(n))) {
                            OutboundAdmission.Accepted -> {
                                progressed = PrnsNativeBridge.nativeBleCommitControlOut(link.connId)
                                if (!progressed) {
                ...
                if (!progressed) {
                    generation = PrnsNativeBridge.nativeBleWaitForWork(
                        generation,
    ...
    private fun startDataOutPump() {
        startWorker("data-out") {
            ...
            while (running) {
                ...
                for (link in links.values) {
                    direct.clear()
                    val n = PrnsNativeBridge.nativeBleDataOut(link.connId, direct)
                    if (n > 0) {
                        pending = true
                        ...
                        when (deliverData(link, scratch.copyOf(n))) {
                            OutboundAdmission.Accepted -> {
                                progressed = PrnsNativeBridge.nativeBleCommitDataOut(link.connId)
                                if (!progressed) {
    ...
    private fun startDialPump() {
        startWorker("dial") {
            ...
            while (running) {
                ...
                direct.clear()
                if (!PrnsNativeBridge.nativeBleNextDial(direct)) {
                    generation = PrnsNativeBridge.nativeBleWaitForWork(generation, 0)
                    continue
                }
                direct.position(0)
    ...
    private fun startL2capOpenPump() {
        startWorker("l2cap-open") {
            ...
            while (running) {
                ...
                direct.clear()
                if (!PrnsNativeBridge.nativeBleNextL2capOpen(direct)) {
                    generation = PrnsNativeBridge.nativeBleWaitForWork(generation, 0)
                    continue
                }
                direct.position(0)
        ...
        object : BluetoothGattCallback() {
                ...
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    if (!linkedConnIds.remove(connId)) {
                        parseMac(address)?.let { octets ->
                            val direct = ByteBuffer.allocateDirect(6)
                            direct.put(octets)
                            if (!PrnsNativeBridge.nativeBleDialFailed(direct)) {
            ...
            override fun onDescriptorWrite(
                ...
                if (descriptor.characteristic.uuid == COLUMBA_TX) {
                    ...
                    if (octets != null) {
                        if (!PrnsNativeBridge.nativeBleColumbaLinkUp(
                            connId,
                            directBufferOf(octets),
                            RSSI_NONE,
                            true,
                ...
                if (octets != null) {
                    val direct = ByteBuffer.allocateDirect(6)
                    direct.put(octets)
                    if (!PrnsNativeBridge.nativeBleLinkUp(connId, direct, RSSI_NONE, true)) {
            ...
            override fun onCharacteristicChanged(
                ...
                if (deliverGattInbound(connId, dataLane, value) == PrnsNativeBridge.BLE_INGRESS_CLOSED) {
                    closeLink(connId)
                }
            }
        }
    ...
    private fun closeLink(connId: Int) {
        ...
        ownedWorkers.forEach(Thread::interrupt)
        PrnsNativeBridge.nativeBleDisconnected(connId)
    }
    ...
    private companion object {
        private const val TAG = "AppliancePrnsBle"
