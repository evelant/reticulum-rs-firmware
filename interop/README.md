...
## Python LXMF 1.0.1 foundation lane

`vectors/lxmf-1.0.1-v1.json` is generated by Python LXMF 1.0.1 itself at the
exact forward-reference revision recorded in the corpus. It covers binary
title/content, rich nested MessagePack fields, exact message IDs and
signatures, 32-byte proof-of-work stamps, 16-byte ticket stamps with
`FIELD_TICKET`, opportunistic/Direct/Resource ingress forms and the Python
packet/Resource selection boundaries. Negative fixtures freeze Python's
signature, source-identity, truncation and stamp-validation outcomes.

Install the pinned Python authority and check both the committed bytes and the
independent assertions with:

```sh
python3.13 -m pip install \
  --target artifacts/phase0/lxmf-1.0.1-python \
...
PYTHONPATH=interop/python:artifacts/phase0/lxmf-1.0.1-python \
  python3.13 -m unittest -v interop/python/test_lxmf_1_0_1_vectors.py
```

The generator rejects a version or source-digest mismatch before emitting
bytes. It imports no Rust or Rete implementation. Its documented Rete
incompatibilities are expectations for later Rust conformance tests, not a
claim that this Python lane executed Rete. Propagation encryption, Paper,
router persistence/message sync and RNS Resource segmentation remain deferred.
