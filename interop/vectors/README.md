...
## Python LXMF 1.0.1 foundation

`lxmf-1.0.1-v1.json` is the schema-1 corpus generated by
`../python/generate_lxmf_1_0_1_vectors.py`. The generator runs the exact Python
LXMF 1.0.1 and RNS 1.3.5 sources pinned by
`../python/requirements-lxmf-1.0.1.txt`, verifies the authority's source
digests, and records complete provenance in the output.

Eight deterministic messages cover the Precursor Python known-answer input,
invalid-UTF8 binary title/content, nested known and unknown MessagePack fields,
valid 32-byte proof-of-work and 16-byte ticket-derived stamps, `FIELD_TICKET`,
and both sides of the opportunistic and Direct/Resource thresholds. Every
message records its destination-DATA, LinkData or ResourceComplete ingress form
and the normalization back to complete LXMF bytes. Six mutations cover changed
signatures, content and source hashes, truncation, and independently invalid
PoW/ticket stamps.

This is a wire and transport-neutral ingress corpus, not evidence for durable
LXMF storage, retries, propagation-node sync, Paper messages or actual RNS
Resource segmentation. Rust-self-generated LXMF fixtures are not treated as a
