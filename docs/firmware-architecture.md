...
`f6f5fb0637d00691e09fa0105be4df902405fee4`; the current `6612f4d` host
regressions cover exact reverse-interface/proof behavior, typed transactional
reverse admission, a deterministic three-node relay flow and the keepalive
lifecycle, but require a new powered rerun. These are instrumented,
...
| --- | ---: | --- | --- | --- |
| `reference/rete` | `6612f4d` integration fork (designated tag `firmware-pin-6612f4d`), based on `9bcb7d3e` | MIT OR Apache-2.0 declared in Cargo/README; license texts absent | Runtime-agnostic `no_std` RNS, bounded transport storage, RNode LoRa interface, compiling Embassy ESP32-S3/SX1262/Wi-Fi example, canonical local LINKREQUEST validation, transactional owned/relay-Link and H2 reverse admission, typed capacity/conflict rejections, endpoint announce policy, caller-owned DATA preparation, allocation-atomic receipt terminals, full-hash/Link-ID-bound DATA/channel terminal candidates, owned-H2 local dispatch, exact path/reverse/Link forwarding, fail-closed LRPROOF validation, authenticated owned-Link interface binding, and Python-compatible keepalive lifecycle | Provisional RNS foundation; pre-release, Resource is whole-buffered, H1 interface-role classification remains explicit product work, persisted paths remain inactive without stable interface rebinding, a shared Tokio Hub lacks endpoint-aware Link identity, channel retransmission receipt replacement and automatic timeout `LINKCLOSE` emission remain open, and current LXMF is wire-incompatible; missing canonical license files are notice/provenance hygiene, not an evaluation blocker |
...
| Exact `6612f4d` selected routing/receipt/keepalive/LXMF validation set | 621 pass | The four current library targets are 167 transport, 134 stack, 143 LXMF and 84 daemon tests, totaling 528. This pin was also run through 92 transport integration tests (9 computed-vector, 43 forwarding, 35 Link-integration and 5 path-request) and one stack integration test, producing the selected 621-test total. It is not a count of every nested workspace test target. All-target host and portable `no_std` checks remain separate compile gates |
| `reticulum-conformance-rete` project runner | 184 pass | The 112 released-vector, adapter and direct-Link checks are joined by 40 exact keepalive lifecycle checks and 32 deterministic three-node A--B--C checks covering learned transport routing, exact-interface LINKREQUEST/LRPROOF/LRRTT relay, bound endpoint output, encrypted channel DATA/proof delivery, receipt completion and separate owned/relay/reverse capacity. This is project-side conformance, not powered or live-Python multi-hop qualification |
...
`6612f4d91593a8f26a77576bd56329a08b8d70ea` closes that generic lifecycle. DATA
...
rather than a redundant reverse entry. The 184-check project conformance run
exercises a complete A--B--C relayed Link handshake and encrypted channel proof
flow. Remaining routing work includes live-Python multi-hop, explicit H1
interface roles, Python-parity `expected_hops` for pending owned Links, and
...
Valid bound Link traffic also revives Stale. A channel retransmission
still creates freshly randomized ciphertext without replacing the receipt
keyed to the original packet hash, and automatic timeout removal still emits
no `LINKCLOSE`; those are separate open residuals.

This exact pin's default E290 release build links and packages successfully.
The 767,696-byte merged image uses 702,160/6,291,456 application bytes (11.16%)
and has SHA-256
...
unquantified. The release gate requires final little-endian Xtensa executables
with resolved compiler stack-size records, caps the maximum frame at that
52,752-byte value, and preserves the current default/HIL linked usable stacks
at 170,984/170,288 bytes with exact 60-byte guard offsets. This preserves the
static inputs to the predecessor baseline's 19,268-byte calculation; it does
not continuously reproduce the powered watermark. The zero PSRAM allocation
...
The current linked default/HIL pair keeps the 52,752-byte maximum frame and
170,984-byte default usable stack; the initialized proof record moves the HIL
usable stack down exactly 192 bytes to 170,288. The default ELF has no
proof-trace symbol; the HIL ELF has exactly one initialized 192-byte record.
The final 779,184-byte HIL image passes build-only qualification. Its
...
changing proof scheduling, retry, or radio receive behavior.
...
3. Preserve the 129-test default, 135-test commit-fault HIL, and 145-test
   ...
   host-client, 252-test xtask, 54-test Rete integration, 184-check Rete
