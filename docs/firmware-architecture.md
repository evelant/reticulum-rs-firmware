...
lifetime job handoff are qualified, while live external admission awaits
...
exist, but there is no credential persistence/pairing, composed external API
...
3. Preserve the 25-test E290 host composition gate, including the authenticated
   ...
   store plus exclusive physical-presence pairing/rate policy; resolve whether
   durable acceptance records the lease's credential generation, authority
   revision and policy version; then compose authority/framing/session/boot-
