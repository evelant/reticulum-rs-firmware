...
| `reticulum-device-api-pairing`, host, generic bare-metal and ESP32-S3 Xtensa | Pass (portable live-pairing core; not yet product-composed) | The featureless allocation-free ADR 0010 core freezes zero-session/zero-tag Begin, ProofStart, Activate-continuation and identifier-free AbortCurrent records for the wired developer profile. Credential ID/generation remain typed through the transcript, continuation verification and activation confirmation; full HMAC-SHA256 proofs use distinct client/activation domains and constant-time verification. Project-owned PSKs, challenges, proofs, confirmations, codec payload scratch, decoded records and framed wire owners zeroize; secret-bearing public owners are neither copyable, cloneable nor debuggable. Upstream RustCrypto SHA-256/HMAC contexts do not implement `Zeroize`, so their internal state remains an explicit backend residual. Thirteen Rust tests plus four compile-fail doctests and five independent standard-library Python tests cover all eight successful COBS flights, exact KATs, every transcript byte/role/length, result vocabularies, malformed profiles/shapes/references, substituted continuations, secret-owner drop glue and proof rejection. The exact reviewed graph is HMAC, SHA-256, zeroize, credential authority and framing plus test-only hex; it contains no policy, entropy, store, USB, task handoff, logical API, session, firmware, Rete or radio edge and remains forbidden from product/HIL graphs until composed |
...
and secret zeroization. E290 durable lifecycle mutation, entropy and
secret-bearing handoff composition plus the authenticated session/API bearer
