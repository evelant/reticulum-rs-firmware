...
`f6f5fb0637d00691e09fa0105be4df902405fee4`; the current `fb96ac1` host
regressions cover exact reverse-interface/proof behavior, typed transactional
reverse admission and a deterministic three-node relay flow, but require a new
...
| Exact `fb96ac1` selected routing/receipt/LXMF validation set | 614 pass | The four current library targets are 165 transport, 132 stack, 143 LXMF and 84 daemon tests, totaling 524. This pin was also run through 89 transport integration tests (9 computed-vector, 43 forwarding, 32 Link-integration and 5 path-request) and one stack integration test, producing the selected 614-test total. It is not a count of every nested workspace test target. All-target host and `thumbv6m-none-eabi` checks remain separate compile gates |
| `reticulum-conformance-rete` project runner | 144 pass | The 112 released-vector, adapter and direct-Link checks are joined by 32 deterministic three-node A--B--C checks covering learned transport routing, exact-interface LINKREQUEST/LRPROOF/LRRTT relay, bound endpoint output, encrypted channel DATA/proof delivery, receipt completion and separate owned/relay/reverse capacity. This is project-side conformance, not powered or live-Python multi-hop qualification |
...
`fb96ac102be4b2a2697484cd5b5c1e3f1adea6a2` closes that generic lifecycle. DATA
...
rather than a redundant reverse entry. The 144-check project conformance run
...
isolation. Independent interoperability defects also remain: Python keepalives
use unencrypted `0xff` request/`0xfe` reply bytes with only the initiator
requesting, while Rete currently encrypts them and lets either side initiate;
and a channel retransmission creates freshly randomized ciphertext without
replacing the receipt keyed to the original packet hash. These remain separate
fixes from interface binding.

This exact pin has no retained E290 ELF, flashed-image readback or powered
proof. Historical artifact and powered records elsewhere in this document stay
bound to the project and Rete revisions they name.
...
3. Preserve the 129-test default, 135-test commit-fault HIL, and 145-test
   ...
   host-client, 248-test xtask, 53-test Rete integration, 144-check Rete
