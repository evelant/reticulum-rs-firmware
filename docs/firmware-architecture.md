...
initialization and canonical reattachment. Current source passes 156 default
E290 host-library tests, 162 with the opt-in inbox commit-fault HIL feature, 175
with the opt-in runtime-measurement HIL feature, 56 focused host-client tests
inside the 272-test xtask suite, a focused 95-test plus one compile-fail-doctest
Rete regression lane, and target gates.
...
authenticated-malformed teardown. The current `90570ca` pin adds the precise
LRRTT lifecycle/timing contract but still requires root validation, an E290
build, and a new powered rerun.
...
| `reticulum-submission-runtime`, host, generic bare-metal and ESP32-S3 Xtensa | Pass | The executor- and transport-neutral runtime owns boot recovery and the durability-first live scheduling order over backend-independent `StorageActor` plus `SubmissionNodePort`. Physical calls borrow exact bound access, while frame observations stay backend-free; binding is checked before recovery phase change, node preparation, or acknowledgement. It persists the no-replay preparation barrier before native DATA preparation and withholds terminal/recovery release behind committed records. Eight focused fake-NOR tests cover the full barrier/frame/terminal/ack order, lost-write reconciliation, permanent frame-persistence failure without a false durable result or acknowledgement, pre-frame terminal pressure, recovery-ack pressure, retry classification, reboot recovery, and wrong-binding rejection; strict host/generic/Xtensa gates pass. The E290 image keeps it resident, and the exact frame request/durable-echo path plus its one-entry composition cap pass cross-layer host tests. Its first authenticated external admission edge is now powered-qualified through acceptance, packet preparation, LoRa DATA/proof, durable `Delivered`, and terminal status after USB re-enumeration. |
| `reticulum-device-api-adapter`, default/experimental/dependency-unified host and ESP32-S3 Xtensa | Pass | The allocation-free authenticated dispatcher exposes public capabilities and the copy-only primary destination, plus principal-scoped submission status; it fails closed during port ambiguity/fault and restricts advertised operations to its local build. Identity is supplied as a node-owned scalar outside `SubmissionPort`, so the read performs no port I/O. The target-safe `experimental-rns-data` path converts one authorized borrowed payload plus validated non-wire dispatch provenance into an owned schema-2 acceptance candidate through the narrow port. The separate `experimental-rns-inbox` path accepts only authenticated principals, then obtains status or an owned bounded peek response through a read-only `InboundMailboxPort`; it adds no persisted permission bit and maps empty to `NotFound`. Focused tests and strict Clippy pass across the relevant host and bare-metal profiles without giving the adapter an actor, journal, raw inbox store, or flash capability. `ProductStorageCoordinator` implements disjoint short-lived submission and inbox views in the E290 graph. The node handoff lane calls them synchronously only after current-authority revalidation; powered API 1.1 supplied sequential submission/status requests and powered API 1.2 supplied bounded inbox status/peek. |
...
| `reticulum-device-api-session`, host, generic bare-metal and ESP32-S3 Xtensa | Pass (portable core plus minimal E290 USB bearer composition) | The allocation-free server and public `no_std` client typestates freeze the USB Serial/JTAG-only HKDF/HMAC qualification transcript, full mutual proofs, direction-separated 128-bit record tags, exact-next sequences, partial-TX ownership, and a single-request typestate. The server's non-cloneable grant contains credential ID/generation and session routing facts but no PSK, principal or permissions; it revalidates against the portable device-owned authority to obtain a borrowing dispatch lease. The E290 USB task composes one handshake per connection and one request in flight, with fault-until-reset behavior; the host utility now drives either one-shot operations or sequential submit/status flights in the same authenticated session. Resumption, retries, close records, encryption, rate/attempt policy, repeated attempts, and concurrency are deferred. The portable crate itself still contains no credential persistence, pairing/rate policy, USB HAL, Rete or radio edge. Capabilities, identity, durable submission, repeated status, and a fresh post-re-enumeration status session are powered-qualified; the deferred behavior remains open. |
...
`90570cafc812b3025011cb690ec74a27f287cb3f` retains that generic lifecycle fix. DATA
...
channel receipts. Adaptive channel windows can still exceed the product's `L`
...
timestamps unchanged. Automatic timeout removal still emits no `LINKCLOSE`;
shared-Hub endpoint/reincarnation identity is the other owned-Link routing
residual.
...
`SubmissionPort` and `InboundMailboxPort` semantic seams under independent
one-entry qualification caps, and the minimal
...
full LXMF routing/propagation and store lifecycle, local client delivery, and
product-capacity policy remain open. Authenticated raw RNS DATA submission and
peer proof are now qualified; the newer opportunistic LXMF receive composition
is not yet powered-qualified.
The current 8 ms missed-SOF suspension
retains its epoch and sequence until bus reset; broader powered lifecycle
qualification is still required.

The E290 library's 156-test default host suite, its 162-test opt-in inbox
commit-fault HIL profile, and its 175-test opt-in runtime-measurement HIL
profile now qualify that three-task software composition,
...
views under independent one-entry qualification caps, neither of which is
product capacity. A static depth-one authenticated job/reply handoff is split
...
task feeds this lane through a minimal single-flight session manager: one
handshake per connection, one request at a time, and fault-until-reset behavior.
...
opportunistic destination DATA and sends a retained proof through the ordinary
supervisor only after a new commit or a fresh retransmission recognized as
`AlreadyDurable`. The LXMF store and raw
inbox remain disjoint short-lived coordinator views. Local Link admission is
disabled for both registered destinations, so Link/Resource events cannot enter
an unowned local queue. A mounted service emits a separately signed
`lxmf.delivery` discovery announce with canonical LXMF 1.0.1 `[nil, nil, []]`
application data unless a clean fault has disabled that service. The product
...
does not currently suppress discovery. Local destination path-
request response, app-level receipt/request handling, direct delivery and RNS
Resource owners remain future consumers. Resource ingress stays rejected before
native mutation until Rete can preflight network-controlled allocation, bound
output windows, and stream accepted bodies into durable blob handles.
...
derived `lxmf.delivery` destination with local Links disabled and per-destination
`Retain`. Opportunistic destination DATA crosses the portable ingress and durable
owner; only a new commit or a fresh retransmission recognized as
`AlreadyDurable` makes its packet proof ready for the ordinary transport-neutral
supervisor. Volatile delayed-proof state is not reconstructed after reboot.
...
of spending the bootstrap attempt. Powered store
and remote durable-delivery qualification remain open, as do Link/Resource,
outbound/direct/propagated LXMF, ticket/PoW requirements, reclamation and client APIs.
...
for the outbound submission composition profile, not product capacity. The
...
plus read-only `InboundMailboxPort`.
...
open. Current source now narrows the application-
...
newer mount-gated opportunistic LXMF receive path is an early bounded Phase 3
source milestone, but it does not close Phase 2 or qualify Phase 3's full
router/message-store deliverables on powered hardware.
...
`lxmf.delivery`, validates signed opportunistic DATA against recalled source
identity, durably commits new messages or recognizes fresh retransmissions in
its separate 2 MiB store, and releases each event's retained proof through the
ordinary transport-neutral supervisor. Its fixed volatile owners are a tunable
E290 concurrency profile,
...
3. Preserve the 156-test default, 162-test commit-fault HIL, and 175-test
   runtime-measurement HIL E290 host-library compositions, 56 focused
   host-client, 272-test xtask, 61-test Rete integration, 647-check schema-2
   ...
   opportunistic-admission seam, its separate durable semantic owner, and the
...
destination with `Retain`, supplies explicit event/proof/store owners, and drains
ready proofs through the ordinary transport-neutral supervisor without sending
from durable ingress.
