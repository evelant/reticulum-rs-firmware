...
124 E290 host-library tests, 29 focused host-client tests, and target gates.
...
The current 748,016-byte image, SHA-256
`4864180ab1d51081758ec3bec53068d6c75316209a2ccc269a0aad48c210fe2c`,
then matched exact readback on MAC `ac:a7:04:e1:3e:88` and completed
button-confirmed initialization, durable Active generation 3, exact Active
partition readback, hard reset, and authenticated capabilities. The permanent
source graph now also composes the feature-free
...
measures 641,419 bytes text, 3,596 bytes initialized data, 469,232 bytes BSS,
and 1,114,247 bytes total by GNU size. Its 748,016-byte merged image has SHA-256
`4864180ab1d51081758ec3bec53068d6c75316209a2ccc269a0aad48c210fe2c`;
exact address-zero readback matched on MAC `ac:a7:04:e1:3e:88`, followed by the
qualified initialize/pair/reboot/capabilities path above. Live external
admission now reaches one authenticated request; authenticated RNS DATA
submission and broader lifecycle/session qualification remain next. Host,
portable-target, ESP32-S3 build,
...
| `reticulum-heltec-vision-master-e290-node`, host and ESP32-S3 Xtensa | Pass (current source/test composition plus bounded powered initialization/authentication proof) | The permanent graph composes the transport-neutral node, concrete LoRa actor, sole USB/GPIO owner, credential initialization and live pairing, authenticated session/admission, static request/reply handoff, and current-authority logical dispatch. The deliberately minimal USB profile permits one handshake per connection and one request in flight; a fault is terminal until reset/re-enumeration. It defers resumption, protocol retries, close records, encryption, rate limiting/attempt policy, repeated handshake attempts, and concurrency. The session/admission boundary is bearer-neutral for later BLE/Wi-Fi adapters and is not a Reticulum packet interface; concurrent bearers still require globally unique bearer-qualified epochs or disjoint reply lanes under one exclusivity coordinator. The current 748,016-byte merged image matched exact address-zero readback on MAC `ac:a7:04:e1:3e:88`; that board completed physical initialization, pairing through durable Active generation 3, exact Active credential-partition readback, hard reset, and an authenticated `system.capabilities` request/reply. Earlier images matched both boards and left both credential partitions erased. Exact Pending/Abort readbacks, authenticated submission, controlled peer RX/DATA, suspend/resume, cuts, runtime high-water, and full qualification remain open. Current test counts, size measurements, image digest, and exact historical evidence are recorded in the E290 runbook. |
...
semantic seam under a one-entry qualification cap, and the minimal
...
durable offer. On one board, the current image then completed a physical hold,
...
authenticated RNS/LXMF submission, message storage, local client delivery, and
product-capacity policy remain open. The current 8 ms missed-SOF suspension
retains its epoch and sequence until bus reset; broader powered lifecycle
qualification is still required.

The E290 library's 124-test host suite now qualifies that three-task software
...
authenticated session manager. One rebooted authenticated capabilities exchange
is powered-qualified; submission and the deferred session behavior remain open.
...
credentials erased. The current image has additionally completed a confirmed
...
source-composed and its rebooted capabilities happy path is powered-qualified;
authenticated submission, mutation faults, and deferred session behavior remain
open.
...
authenticated records. The current image later completed button-confirmed
...
session/bearer is also source-composed, and its capabilities happy path is
powered-qualified. Authenticated submission, remaining live-lifecycle states,
...
3. Preserve the 124-test E290 host-library composition gate, 29 focused host-
   ...
   exact preceding-image readback, hard-reset reattachment, and current one-board
   initialize/pair/Active-readback/reboot results; next complete exact
   ...
   submission-port view, minimal single-flight USB session bearer, and powered
   capabilities handshake/request/reply; next qualify authenticated submission
   through the durable runtime and real LoRa peer. Keep the admission and
