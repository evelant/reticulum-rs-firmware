...
123 E290 host-library tests, 29 focused host-client tests, and target gates.
...
workflows. The last powered 718,688-byte authenticated-node-foundation image, SHA-256
...
one handshake per connection, one request at a time, and terminal fault handling
...
concurrency are intentionally deferred. Credential selection, admission
handoff, and node dispatch remain transport-neutral. The qualification crypto
suite is currently USB Serial/JTAG-only; later BLE/Wi-Fi bindings can reuse the
ownership boundary after their suites are implemented and qualified. Before
...
not merge colliding epochs into the current singleton lane. The current bearer-linked source
measures 640,587 bytes text, 3,596 bytes initialized data, 469,232 bytes BSS,
and 1,113,415 bytes total by GNU size. Its 747,184-byte merged image has SHA-256
...
`initialization-required`, and both credential partitions remained erased.
This is current package/readback and bootstrap evidence, not an authenticated
exchange. Live external admission therefore
awaits powered credential initialization and authenticated
handshake/request/reply proof; host, portable-target, ESP32-S3 build,
...
| `reticulum-heltec-vision-master-e290-node`, host and ESP32-S3 Xtensa | Pass (current source/test composition plus bounded powered package/bootstrap regression) | The permanent graph composes the transport-neutral node, concrete LoRa actor, sole USB/GPIO owner, credential initialization and live pairing, authenticated session/admission, static request/reply handoff, and current-authority logical dispatch. The deliberately minimal USB profile permits one handshake per connection and one request in flight; a fault is terminal until reset/re-enumeration. It defers resumption, protocol retries, close records, encryption, rate limiting/attempt policy, repeated handshake attempts, and concurrency. The session/admission boundary is bearer-neutral for later BLE/Wi-Fi adapters and is not a Reticulum packet interface; concurrent bearers still require globally unique bearer-qualified epochs or disjoint reply lanes under one exclusivity coordinator. The current 747,184-byte merged image matched exact address-zero readbacks on both boards, both served sequence-zero bootstrap status, and both credential partitions remained erased. Powered credential initialization, activation, authenticated handshake/request/reply, controlled peer RX/DATA, suspend/resume, cuts, runtime high-water, and full qualification remain open. Current test counts, size measurements, image digest, and exact historical evidence are recorded in the E290 runbook. |
...
| `reticulum-submission-runtime`, host, generic bare-metal and ESP32-S3 Xtensa | Pass | The executor- and transport-neutral runtime owns boot recovery and the durability-first live scheduling order over backend-independent `StorageActor` plus `SubmissionNodePort`. Physical calls borrow exact bound access, while frame observations stay backend-free; binding is checked before recovery phase change, node preparation, or acknowledgement. It persists the no-replay preparation barrier before native DATA preparation and withholds terminal/recovery release behind committed records. Eight focused fake-NOR tests cover the full barrier/frame/terminal/ack order, lost-write reconciliation, permanent frame-persistence failure without a false durable result or acknowledgement, pre-frame terminal pressure, recovery-ack pressure, retry classification, reboot recovery, and wrong-binding rejection; strict host/generic/Xtensa gates pass. The E290 image keeps it resident, and the exact frame request/durable-echo path plus its one-entry composition cap pass cross-layer host tests. The minimal authenticated USB lane is now source-composed as its first external admission edge; powered proof remains open. |
| `reticulum-device-api-adapter`, default/`experimental-rns-data`/dependency-unified host and ESP32-S3 Xtensa | Pass | The allocation-free authenticated dispatcher exposes public capabilities and principal-scoped status, fails status closed during port ambiguity/fault, and restricts advertised operations to its local build. The target-safe `experimental-rns-data` path converts one authorized borrowed payload plus validated non-wire dispatch provenance into an owned schema-2 acceptance candidate through the narrow `SubmissionPort`, then maps durable acceptance/replay, conflict, capacity and port faults to stable API results without receiving an actor, journal or flash capability. Focused tests and strict Clippy pass across the relevant host and bare-metal profiles. `ProductStorageCoordinator` implements the port in the E290 graph under a one-entry qualification cap. The node handoff lane calls it synchronously only after current-authority revalidation through a disjoint short-lived port view; the source-composed minimal USB endpoint can now supply one request at a time. |
...
The E290 library's 123-test host suite now qualifies that three-task software
...
is terminally quarantined. The USB task now feeds this lane through a minimal
single-flight session manager: one handshake per connection, one request at a
time, and fault-until-reset behavior. The source path exists, but no external
API job has yet reached the node on powered hardware. Separate portable framing,
the featureless pre-authentication
...
establishment, direct flash, node or radio work. BLE/Wi-Fi bearers remain
unimplemented; they can reuse the bearer-neutral admission/handoff boundary
after their session bindings/suites are explicitly implemented and qualified.
...
bearer now exist in source; powered end-to-end proof does not.
...
requires powered live lifecycle pairing and authenticated USB qualification.
The portable session/handoff crates, node-side current-authority dispatch, and
minimal USB session manager are already source-composed. A send
...
dispatch through a credential-disjoint submission-port view. The USB task owns
the other handoff endpoint and feeds it through the minimal single-flight
authenticated session manager; powered qualification remains open.
...
scrubbed reattachment. The minimal authenticated USB session/API bearer is now
source-composed; powered mutation/fault and handshake/request/reply
...
and has not yet run end-to-end on powered hardware. BLE/Wi-Fi bearers remain
...
credential readbacks remained entirely erased. The last powered 718,688-byte
...
session/bearer is also source-composed, while successful powered live lifecycle
mutation and authenticated exchange, controlled peer RX/DATA,
...
The last powered 718,688-byte authenticated-node-foundation image, SHA-256
`e20f6191cb2bfa78fbd7f3d588eb418913da3f1f89e3b80a4db0a28abaf414ea`,
...
3. Preserve the 123-test E290 host-library composition gate, 29 focused host-
   client tests, and full 176-test xtask gate, including credential boot
   ...
   submission-port view, and minimal single-flight USB session bearer; then
   qualify its powered handshake, request, and reply. Keep the admission and
   node-dispatch boundary reusable by later BLE/Wi-Fi bearers, with a separately
   implemented and qualified session suite for each binding. Select the real capacity policy
