...
`14c7b4955a1ff6903e87cc40b42498f7869b6f4f`; `firmware-pin-14c7b49` is its
...
remains bound to the project and Rete revisions recorded with it.
...
performs 235 checks: 112 provenance, identity, signed-announce, packet-parse,
...
These regressions complement, but do not replace, the project's 235 conformance
...
A malformed or nonnumeric authenticated LRRTT is rejected before establishment
or hop mutation, emits a best-effort encrypted `LINKCLOSE` on the bound
interface, reclaims Link-owned state, and produces one terminal close even if
the packet is replayed. Active-responder repeat LRRTT remains a nonblocking
state-machine gap until Rete retains immutable `request_time`; RTT timing also
remains whole-second `u64`/`f32` rather than Python's double-precision
...
and pre-dedup wrong-hop LRPROOF rejection. The 235-check project runner
