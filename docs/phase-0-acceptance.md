...
`8b5d65283cd370dee4cbb17594ef9c88d2805416`; `firmware-pin-8b5d652` is its
...
evidence. Its 775,104-byte merged image uses 709,568/6,291,456 application
bytes (11.28%) and has SHA-256
...
performs 195 checks: 112 provenance, identity, signed-announce, packet-parse,
payload, packet-hash, owning-adapter/native-core and direct-Link checks, 8
channel-retry lifecycle checks, 40 exact keepalive lifecycle checks, and 35
...
Separately, the exact pinned Rete selected validation set passes 635 tests: 271
...
library targets total 537 tests; the 97 transport and one stack integration
tests were also run for this pin to reach 635. This is not a count of every
nested workspace test target.
These regressions complement, but do not replace, the project's 195 conformance
...
and decryption. LRRTT payload parity remains a distinct gate: Python sends a
...
authenticated LRRTT can therefore establish the Link and teach its hop count.
Require bounded MessagePack numeric encode/decode, cross-language vectors, and
payload validation before establishment or hop-state mutation.
...
and pre-dedup wrong-hop LRPROOF rejection. The 195-check project runner
