...
`6612f4d91593a8f26a77576bd56329a08b8d70ea`; `firmware-pin-6612f4d` is its
...
evidence. Its 767,696-byte merged image uses 702,160/6,291,456 application
bytes (11.16%) and has SHA-256
...
performs 184 checks: 112 provenance, identity, signed-announce, packet-parse,
payload, packet-hash, owning-adapter/native-core and direct-Link checks, 40
exact keepalive lifecycle checks, and 32 deterministic three-node A--B--C
...
Separately, the exact pinned Rete selected validation set passes 621 tests: 259
transport (167 library plus 92 integration: 9 computed-vector, 43 forwarding,
35 Link-integration and 5 path-request), 135 stack (134 library and one
integration), 143 LXMF library and 84 daemon library tests. The four current
library targets total 528 tests; the 92 transport and one stack integration
tests were also run for this pin to reach 621. This is not a count of every
nested workspace test target.
These regressions complement, but do not replace, the project's 184 conformance
...
valid bound Link traffic also revives the Link. Channel retransmission still
creates a fresh ciphertext hash without replacing the original receipt,
automatic timeout removal still emits no `LINKCLOSE`, and pending-Link
`expected_hops` remains a separate interoperability gate.
...
also not exposed. Channel-receipt and several other internal insertions still
need a complete transactional audit. Destination-DATA receipt, relay-Link and
H2 reverse insertion are no longer in that set at the current pin. This is
...
and `RESOURCE_PRF`. The 184-check project runner
