...
`fb96ac102be4b2a2697484cd5b5c1e3f1adea6a2`; `firmware-pin-fb96ac1` is its
...
proof. Every build artifact and powered result below remains historical
evidence bound to the project and Rete revisions recorded with it.
...
performs 144 checks: 112 provenance, identity, signed-announce,
packet-parse, payload, packet-hash, owning-adapter/native-core and direct-Link
checks plus 29 deterministic three-node A--B--C relayed-Link checks. Rete
...
Separately, the exact pinned Rete selected validation set passes 614 tests: 254
transport (165 library, 9 computed-vector, 43 forwarding, 32 Link-integration
and 5 path-request), 133 stack (132 library and one integration), 143 LXMF
library and 84 daemon library tests. The four current library targets total 524
tests; the 89 transport and one stack integration tests were also run for this
pin to reach 614. This is not a count of every nested workspace test target.
These regressions complement, but do not replace, the project's 144 conformance
...
generation are part of Link state. Python keepalives are unencrypted
initiator-request/responder-reply traffic, unlike Rete's current encrypted
bidirectional exchange; channel retransmission also creates a fresh ciphertext
hash without replacing the receipt for the original packet. These and the
pending-Link `expected_hops` audit remain separate interoperability gates.
...
and `RESOURCE_PRF`. The 144-check project runner
